...
class EditPricingRule extends EditRecord
{
    ...
    {
        // JSON verileri kullanıcı dostu forma dönüştür
        $conditions = $data['conditions'] ?? [];
        $actions = $data['actions'] ?? [];

        // Customer types
        if (!empty($conditions['customer_types'])) {
            $data['customer_types'] = $conditions['customer_types'];
        }

        // Quantity conditions
        if (!empty($conditions['min_quantity'])) {
            $data['min_quantity'] = $conditions['min_quantity'];
        }
        if (!empty($conditions['max_quantity'])) {
            $data['max_quantity'] = $conditions['max_quantity'];
        }

        // Amount conditions
        if (!empty($conditions['min_amount'])) {
            $data['min_amount'] = $conditions['min_amount'];
        }
        if (!empty($conditions['max_amount'])) {
            $data['max_amount'] = $conditions['max_amount'];
        }

        // Days of week
        if (!empty($conditions['days_of_week'])) {
            $data['days_of_week'] = $conditions['days_of_week'];
        }

        // Discount type and value
        if (!empty($actions['discount_percentage'])) {
            $data['discount_type'] = 'percentage';
            $data['discount_value'] = $actions['discount_percentage'];
        } elseif (!empty($actions['discount_amount'])) {
            $data['discount_type'] = 'fixed_amount';
            $data['discount_value'] = $actions['discount_amount'];
        }
    ...
    {
        // Kullanıcı dostu form verilerini JSON formatına dönüştür (CreatePricingRule ile aynı mantık)
        $conditions = [];
        $actions = [];
